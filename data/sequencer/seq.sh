...
# Function to generate sequence for a single file
generate_sequence() {
    py_file="$1"
    base_name=$(basename "${py_file%.py}")
    json_file="${py_file%.py}.json"
    
    # Get the relative path from designpatterns directory
    ...
    # Generate sequence using Python
